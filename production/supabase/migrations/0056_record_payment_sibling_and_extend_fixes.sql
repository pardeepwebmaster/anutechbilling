-- 0056: fix two confirmed record_payment money-correctness bugs
--
-- #1b (RP-03, sibling-subscription clobber): the "subsequent payment" branch did
--   UPDATE subscriptions SET outstanding_amount = <this quote's outstanding>
--   WHERE customer_id = <cust> AND outstanding_amount > 0;
-- For a customer with TWO subscriptions (two separate sales), paying one quote
-- overwrote the OTHER sub's outstanding too. Proven red: sibling 3000 -> 2000.
-- Fix: add subscriptions.quote_id (the originating quote), stamp it when the sub
-- is created, and scope the outstanding UPDATE to that quote only.
--
-- #16 (extend-on-already-renewed -> duplicate subscription): the renewal-sub
-- lookup filtered `renewal_state <> 'renewed'`. Generating a fresh extension quote
-- on an already-renewed sub (renewal_quote_id re-set) then paying it fell through
-- to the NEW-SALE path and created a SECOND subscription. Proven red: 2 subs.
-- Fix: a quote that is some sub's renewal_quote_id is ALWAYS a renewal/extension of
-- THAT sub — drop the renewal_state filter so it rolls the same sub forward.
-- (Idempotency is unaffected: the payment-reference dedup returns before this, and
-- after a renewal completes renewal_quote_id is set null, so no re-match.)
--
-- Everything else in record_payment is identical to migration 0055.

-- 1. Originating quote for a subscription (nullable; legacy subs stay null).
alter table public.subscriptions
  add column if not exists quote_id text references public.quotes(id) on delete set null;

comment on column public.subscriptions.quote_id is
  'The quote whose payment created this subscription. Scopes outstanding-amount updates to the right sub when a customer has multiple subscriptions (bug #1b).';
...
begin
  ...
  -- #16 fix: a quote linked as a sub's renewal_quote_id is ALWAYS a renewal of
  -- that sub (no renewal_state filter) — so extending an already-'renewed' sub
  -- rolls THAT sub forward instead of creating a duplicate subscription.
  ...
  if v_is_first_payment and not v_is_renewal_quote and not v_is_add_seats then
    ...
    if v_first_line is not null then
      ...
      if v_is_annual and v_customer_id is not null then
        ...
        -- #1b fix: stamp the originating quote so subsequent-payment outstanding
        -- updates only touch THIS subscription, never a sibling sale's sub.
    ...
    -- #1b fix: scope to the originating quote's subscription (not all of the
    -- customer's subs) so a sibling sale's outstanding is never clobbered.
