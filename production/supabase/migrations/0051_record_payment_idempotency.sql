-- 0051: record_payment idempotency — stop duplicate/retried payments double-recording
--
-- Problem (audit bug #1/#2): record_payment inserted a payment row UNCONDITIONALLY.
-- Razorpay retries webhooks and operators double-click, so the same payment got
-- recorded twice → double total, double receipt voucher, double subscription, wrong AR.
-- Proven: two calls with the same reference produced 2 rows / 2x amount.
--
-- Fix (idempotency keyed on (tenant_id, quote_id, reference) for received payments):
--   1. Early guard — if a received payment with this exact reference already exists
--      for this quote, return the prior result WITHOUT any side-effects (no new row,
--      no voucher, no subscription, no lead conversion).
--   2. DB backstop — a partial UNIQUE INDEX makes a true concurrent race impossible;
--      the INSERT is wrapped so a unique_violation also returns idempotently.
--
-- Assumption: `reference` is unique per distinct payment. Razorpay = payment id (always
-- unique). For MANUAL payments the UI must supply a unique reference (e.g. cash-<ts>);
-- two genuine manual payments must not reuse the same reference string.
--
-- Verified before apply: zero existing duplicate (tenant_id, quote_id, reference)
-- received payments in prod, so the unique index creates cleanly.

-- ── 1. DB backstop: one received payment per (tenant, quote, reference) ──────────
create unique index if not exists payments_idempotency_uq
  on public.payments (tenant_id, quote_id, reference)
  where status = 'received';

-- ── 2. Idempotent record_payment (0047 body + guard + insert backstop) ──────────
...
declare
  ...
  v_existing_payment      record;   -- 0051: idempotency
begin
  ...
  -- ── 0051 IDEMPOTENCY GUARD ──────────────────────────────────────────────────
  -- If this exact payment (same reference) was already recorded for this quote,
  -- do nothing and return the prior result. Safe to call twice (webhook retries,
  -- double-clicks) without double-recording / double-vouchering / double-subbing.
  ...
  if v_is_first_payment and v_quote.lead_id is not null and v_quote.customer_id is null then
    ...
    insert into public.customers (tenant_id, name, contact_name, contact_email, contact_phone, domain, since, health, notes)
    values (v_tenant_id, v_lead.company, v_lead.contact_name, v_lead.contact_email, v_lead.contact_phone, v_domain, current_date,
            case when p_amount >= v_expected then 85 else 75 end, v_lead.notes)
  ...
  -- 0051: wrap the insert so a concurrent identical call (race that slips past the
  -- guard) hits the unique index and returns idempotently instead of erroring.
  begin
  ...
  if v_is_first_payment and not v_is_renewal_quote then
    ...
    if v_first_line is not null then
      ...
      if v_is_annual and v_customer_id is not null then
        ...
        insert into public.subscriptions (tenant_id, customer_id, customer_name, plan, vendor, seats, mrr,
          start_date, renewal_date, status, outstanding_amount, domain)
        values (v_tenant_id, v_customer_id, v_quote.customer_name, v_plan_name, v_vendor, v_seats,
          greatest(0, round(v_expected / 12.0))::int, current_date,
          (current_date + interval '1 year')::date, 'active', v_outstanding, v_domain)
    ...
    update public.subscriptions set outstanding_amount = v_outstanding
     where tenant_id = v_tenant_id and customer_id = v_customer_id and outstanding_amount > 0;
  end if;
  ...
  if v_is_renewal_quote and v_is_fully_paid then
    ...
    v_new_mrr := greatest(0, round(v_expected::numeric / greatest(v_extension_months, 1)))::int;
