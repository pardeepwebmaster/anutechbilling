-- 0157 — Direct (one-off) invoices: mark a quote as one-off so its payment does
-- NOT spin up a subscription / purchase order.
--
-- Every invoice in ResellerOS is derived from a quote (invoice.quote_id). A
-- "direct invoice" therefore also creates a quote behind it — but for a one-off
-- charge (setup fee, ad-hoc service) we must NOT create recurring artefacts.
-- This adds quotes.is_one_off and guards the subscription-creation branch of
-- record_payment with it. Everything else (advance adjustment, invoice numbering,
-- receipt voucher, idempotency) is unchanged and keeps working.
--
-- Surgical diff vs the live record_payment:
--   • SELECT … INTO v_quote  → also fetch q.is_one_off
--   • subscription guard      → add  AND NOT coalesce(v_quote.is_one_off, false)

alter table public.quotes add column if not exists is_one_off boolean not null default false;
...
begin
  ...
  if v_expected <= 0 then raise exception 'quote % has no amount - cannot record a payment against a zero-value quote', p_quote_id using errcode = 'check_violation'; end if;
  ...
  if v_is_first_payment and v_quote.lead_id is not null and v_quote.customer_id is null then
    ...
    insert into public.customers (tenant_id, name, domain, since, health)
  ...
  -- One-off (direct-invoice) quotes never create a subscription / PO — only the
  -- payment + invoice matter (0157: added  AND NOT is_one_off).
  if v_is_first_payment and not v_is_renewal_quote and not v_is_add_seats and not coalesce(v_quote.is_one_off, false) then
    v_first_line := case
    ...
    if v_first_line is not null then
      ...
      if v_is_annual and v_customer_id is not null then
        ...
        if coalesce((v_first_line->>'bulk')::boolean, false)
          ...
          if v_seats <= 0 then raise exception 'annual line has zero seats (quote %)', p_quote_id using errcode = 'check_violation'; end if;
