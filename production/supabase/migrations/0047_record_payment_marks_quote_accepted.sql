-- 0047: record_payment also marks quote.status='accepted' on full payment
--
-- Problem: when buyer paid via record_payment (e.g., Razorpay test mode)
-- without first hitting the public /quote/<id>/accept page, the quote's
-- `status` stayed at 'sent' even though `payment_status='received'`.
-- Dashboard "Accepted (MTD)" KPI filtered on status='accepted' and missed
-- paid quotes — incorrect revenue figure.
--
-- Semantically: payment received == quote accepted. The status transition
-- should fire automatically as part of the atomic record_payment cascade.
--
-- Change: in the UPDATE quotes block, add `status = 'accepted'` when the
-- payment fully covers the expected amount (v_is_fully_paid). Stays as
-- the existing value otherwise (partial payment shouldn't flip to accepted
...
begin
  ...
  -- NEW (0047): mark quote.status='accepted' when fully paid, regardless of
  -- whether the customer went through the public /quote/<id>/accept page.
  -- Semantically, payment received == acceptance. This fixes the Dashboard
  -- "Accepted (MTD)" KPI which previously missed paid-but-not-clicked-accept
  -- quotes.
