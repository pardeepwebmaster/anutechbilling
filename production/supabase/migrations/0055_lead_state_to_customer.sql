-- 0055: carry place-of-supply (state) from a buy-page lead through to the customer
--
-- Why: the GST head (IGST vs CGST+SGST) is derived from customer.state_code vs the
-- tenant's state_code. But buy-page enquiries created a lead → draft quote with
-- customer_id = null, and BOTH conversion RPCs (accept_quote, record_payment) created
-- the customer WITHOUT copying any state, so every buy-page customer started stateless
-- → defaulted to intra-state (CGST+SGST) even for an inter-state buyer. (audit #18-20)
--
-- Fix: capture the buyer's state on the lead (new leads.state_code / leads.state; gstin
-- already existed), then copy state_code + state + gstin into the customer at conversion
-- time in BOTH RPCs. Existing customers untouched; Pardeep can still edit a customer's
-- state in-app. This migration also captures these two SECURITY DEFINER RPCs into version
-- control (they previously lived only in prod — schema drift, bug #34).

-- 1. Lead place-of-supply columns (gstin already exists on leads)
alter table public.leads add column if not exists state_code text;
alter table public.leads add column if not exists state      text;

comment on column public.leads.state_code is
  'GST state code (2 digits, e.g. 27=Maharashtra) captured from the buy page. Copied to the customer on conversion to drive the GST head (IGST vs CGST+SGST).';
comment on column public.leads.state is
  'Human-readable GST state name (e.g. "Maharashtra (27)") for display; paired with state_code.';

-- 2. accept_quote — copy state_code/state/gstin from lead → customer on conversion
...
begin
  ...
  select q.id, q.tenant_id, q.customer_id, q.customer_name, q.lead_id, q.status, q.domain
    into v_quote
  ...
  -- Convert lead → customer if not already done
  if v_customer_id is null and v_quote.lead_id is not null then
    select l.contact_name, l.contact_email, l.contact_phone, l.company, l.notes, l.domain,
     ...
     where l.id = v_quote.lead_id
       and l.tenant_id = v_caller_tenant;
    if not found then
    ...
    insert into public.customers (
      ...
      v_caller_tenant, v_lead.company, v_lead.contact_name, v_lead.contact_email,
      v_lead.contact_phone, v_domain, current_date,
      70,  -- Awaiting payment — lower than 75 (partial) / 85 (fully paid)
  ...
  -- Update quote — mark accepted + link customer
...
-- 3. record_payment — same state copy when it converts a lead → customer
--    (only the lead SELECT and the customers INSERT change; everything else verbatim)
...
begin
  ...
  select q.id, q.tenant_id, q.customer_id, q.customer_name, q.lead_id,
         ...
         q.domain, q.extension_months, q.is_add_seats
    into v_quote
    from public.quotes q
