-- 0065_record_payment_customer_dedup.sql
-- record_payment: reuse an existing same-email customer on first-payment
-- conversion instead of inserting a duplicate (same root-cause fix as 0064 for
-- accept_quote). Email-based, case-insensitive, tenant-scoped. Only the
-- first-payment lead→customer block changed; everything else is verbatim from
-- 0061. Verified green: dedup reuse, new-sale still creates customer + mrr 8640,
-- idempotency / sibling / renewal unaffected.
...
begin
  ...
  if v_is_first_payment and v_quote.lead_id is not null and v_quote.customer_id is null then
    ...
    -- tenant instead of inserting a duplicate (same buyer => one customer record).
