-- 0052: fix add-seats creating a DUPLICATE subscription (audit bug #3/#4)
--
-- Problem: add-seats (lib/subscriptions/add-seats.ts) already bumps the EXISTING
-- subscription's seats at quote-creation time, then creates a pro-rata quote that
-- is annual + has customer_id set + is NOT a renewal. When that quote is paid,
-- record_payment step 8a ("first payment + not renewal + annual + customer") treats
-- it as a brand-new annual sale and inserts a SECOND subscription → duplicate MRR/ARR,
-- duplicate auto-PO, double renewal reminders, customer over-billed at renewal.
-- Proven on test DB: customer with 1 sub + paid add-seats quote => 2 subs.
--
-- Fix: mark add-seats quotes with `is_add_seats` and skip ALL subscription handling
-- in record_payment for them (no 8a create, no 8b outstanding-update, no auto-PO).
-- The existing sub was already updated by add-seats.ts; record_payment should only
-- record the money and update the quote.
--
-- (Extend quotes are unaffected: they link via renewal_quote_id and roll forward via
--  8c. The extend-on-already-renewed edge, bug #16, is tracked separately.)

alter table public.quotes
  add column if not exists is_add_seats boolean not null default false;
...
declare
  ...
  v_is_add_seats          boolean := false;   -- 0052
begin
  ...
  v_is_add_seats         := coalesce(v_quote.is_add_seats, false);   -- 0052
  ...
  -- 0052: skip ALL subscription handling for add-seats quotes — add-seats.ts
  -- already bumped the existing subscription. Without this guard, step 8a would
  -- create a DUPLICATE subscription (bug #3/#4).
