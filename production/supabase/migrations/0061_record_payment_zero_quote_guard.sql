-- 0061_record_payment_zero_quote_guard.sql
-- Reject recording a payment against a zero/NULL-amount quote (audit bug #27).
--
-- record_payment already guards the PAYMENT amount (`p_amount > 0`), but not the
-- QUOTE's own `amount`. A quote with NULL/0 amount makes `v_expected = 0`, so any
-- payment satisfies `total >= expected` → the quote is marked received/accepted,
-- outstanding 0, and a subscription with mrr 0 is spawned (money "fully paid"
-- against nothing). This adds a guard rejecting such quotes.
--
-- Placement: AFTER the idempotent-replay early-return, so an already-recorded
-- payment still replays cleanly; a ₹0 quote can never be meaningfully paid
-- anyway (p_amount must be > 0, which would exceed a ₹0 expected).
--
-- The function is reproduced verbatim from the live definition (migrations 0047
-- → 0057 lineage) with ONLY the guard block added. Verified green by the SQL
-- regression suite: new-sale mrr-ex-gst, idempotent replay, renewal roll-forward,
-- partial-payment status, sibling-sub scoping (#5), and the new ₹0 reject.
...
begin
  ...
  -- #27: a quote with NULL/0 amount makes any payment look "fully paid" and
  -- spawns a mrr=0 subscription. Reject it here (after the idempotent-replay
  -- check, so already-recorded payments still replay cleanly).
  if coalesce(v_quote.amount, 0) <= 0 then
    raise exception 'quote % has no amount — cannot record a payment against a zero/NULL-amount quote', p_quote_id using errcode = 'check_violation';
  end if;
