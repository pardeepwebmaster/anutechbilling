-- 0076_record_payment_line_start_date.sql
-- ============================================================
-- record_payment: honour a per-line service start date.
--
-- The quote builder now lets the operator set a `start_date` (YYYY-MM-DD) on a
-- line item. When present, the subscription created on first payment starts on
-- that date and the renewal date is (start + 1 year). When absent (blank), we
-- fall back to current_date — byte-identical to the previous behaviour, so all
-- existing quotes and flows are unaffected.
--
-- ONLY the two subscription INSERTs (bulk + single) change: their start_date /
-- renewal_date anchor moves from current_date → v_start. Every other current_date
-- (customers.since, invoice paid_date) is deliberately left untouched. Renewal
-- (roll-forward) and add-seats paths are unchanged.
--
-- Body copied verbatim from the live function (0075) so this is a faithful
-- CREATE OR REPLACE with a minimal, auditable diff.
-- ============================================================
...
declare
  ...
  v_start                 date;   -- 0076: per-line service start (falls back to current_date)
begin
  ...
  if v_is_first_payment and not v_is_renewal_quote and not v_is_add_seats then
    ...
    if v_first_line is not null then
      ...
      if v_is_annual and v_customer_id is not null then
        ...
        -- 0076: service start = per-line start_date if the operator set one,
        -- else the payment date (backward-compatible with older quotes).
