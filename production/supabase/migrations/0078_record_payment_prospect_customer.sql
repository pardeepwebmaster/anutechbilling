-- 0078_record_payment_prospect_customer.sql
-- ============================================================
-- record_payment: also convert a TYPED PROSPECT into a customer.
--
-- Before this, a customer was auto-created on first payment only when the quote
-- was linked to a lead (lead_id not null). A quote typed directly for a new
-- prospect (no lead, no existing customer) recorded the payment but created no
-- customer and no subscription — contradicting the builder's "we'll auto-create
-- the customer when they pay" promise, and leaving the money-spine incomplete.
--
-- This adds ONE branch: first payment + customer_id null + lead_id null → create
-- the customer straight from the quote's customer_name (+ domain). The existing
-- subscription / PO / quote-link logic then runs unchanged (it only needed a
-- non-null customer_id). Lead-linked and existing-customer paths are untouched.
--
-- Body copied verbatim from the live function (0076) so the diff is a single,
-- auditable inserted `elsif`.
-- ============================================================
