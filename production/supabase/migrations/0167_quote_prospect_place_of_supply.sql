-- 0167_quote_prospect_place_of_supply.sql
-- ============================================================
-- Carry a TYPED PROSPECT's place-of-supply (state) through to the customer.
--
-- Background (mirrors migration 0055, which fixed the SAME class of bug for the
-- buy-page / lead → customer path):
--   The GST head on a tax invoice (IGST inter-state vs CGST+SGST intra-state) is
--   derived in generate_invoice() from customers.state_code vs the tenant's
--   state_code. A quote typed directly for a NEW prospect (no lead, no existing
--   customer) went through record_payment's 0078 "typed prospect" branch, which
--   created the customer from customer_name + domain ONLY — no state_code. So an
--   inter-state prospect's first invoice silently defaulted to CGST+SGST when it
--   should have been IGST. Wrong tax heads on a GST document = a compliance defect
--   (even though the 18% total is unchanged).
--
-- The builder already CAPTURES the prospect's state (prospectStateCode) and drives
-- the on-screen split from it — but the value was never persisted, so it was lost
-- the moment the quote was saved. This migration:
--   1. Adds prospect_state_code / prospect_state / prospect_country to `quotes` so
--      the captured place-of-supply survives to payment time.
--   2. Recreates record_payment so its typed-prospect branch copies state_code /
--      state / country onto the auto-created customer (exactly like the lead branch
--      already copies l.state_code / l.state).
--
-- Body copied verbatim from the live function (0078) so the diff is auditable:
--   • the quote SELECT gains q.prospect_state_code, q.prospect_state, q.prospect_country
--   • the typed-prospect INSERT gains state_code, state, country
-- Every other path (lead-linked, existing-customer, renewal, add-seats, bulk) is
-- untouched.
-- ============================================================

-- ── 1. Quote place-of-supply columns (only meaningful for prospect quotes) ────
alter table public.quotes add column if not exists prospect_state_code text;
alter table public.quotes add column if not exists prospect_state      text;
alter table public.quotes add column if not exists prospect_country     text;

comment on column public.quotes.prospect_state_code is
  'GST state code (2 digits, e.g. 27=Maharashtra) captured on the quote builder for a TYPED prospect with no customer record yet. Copied to the customer by record_payment on first payment to drive the GST head (IGST vs CGST+SGST). Null when a real customer_id is picked (that customer carries its own state).';
comment on column public.quotes.prospect_state is
  'Human-readable GST state name paired with prospect_state_code (display).';
comment on column public.quotes.prospect_country is
  'Typed prospect country (default India). Non-India = export; copied to the customer so future quotes detect export correctly.';

-- ── 2. record_payment — copy state/country when converting a TYPED PROSPECT ───
...
begin
  ...
  -- 0167: also pull the captured prospect place-of-supply (state/country) so the
  -- typed-prospect branch below can stamp it onto the new customer.
  ...
  if v_is_first_payment and v_quote.lead_id is not null and v_quote.customer_id is null then
    ...
    -- 0078: typed prospect — no lead, no existing customer. Create the customer
    -- straight from the quote so the first payment still converts into a real
    -- customer (+ subscription + PO), matching the builder's promise.
    -- 0167: also carry the captured place-of-supply (state_code/state) + country
    -- so the tax invoice gets the correct GST head (IGST vs CGST+SGST) and future
    -- quotes detect export correctly — instead of a stateless intra-state default.
    ...
    if v_first_line is not null then
      ...
      if v_is_annual and v_customer_id is not null then
        ...
        if coalesce((v_first_line->>'bulk')::boolean, false)
          ...
          v_bulk_pool := greatest(0, round(coalesce(v_quote.subtotal, v_expected) / 12.0))::int;
          v_running_mrr := 0;
          v_idx := 0;
          ...
          values (v_tenant_id, v_customer_id, v_quote.customer_name, v_plan_name, v_vendor, v_seats,
            greatest(0, round(coalesce(v_quote.subtotal, v_expected) / 12.0))::int, v_start,
            (v_start + interval '1 year')::date, 'active', v_outstanding, v_domain, p_quote_id)
          returning id into v_new_sub_id;
          v_subscription_created := true;
