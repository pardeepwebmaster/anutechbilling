-- 0075: BULK ORDERS — one quote/invoice expands into one subscription PER DOMAIN.
--
-- A reseller customer can bulk-order Google Workspace on many domains (each its
-- own seat count) under ONE quote + ONE invoice. Before this, record_payment read
-- only line_items[0] and created exactly one subscription per quote — a bulk order
-- would lose all but the first domain.
--
-- A "bulk line" is a quote line with bulk=true and a domains:[{domain,seats}] array
-- (qty = Σ seats). On the FIRST payment of such a quote, this RPC loops the domains
-- and inserts one subscription per domain, all stamped with quote_id (the order key).
--
-- MONEY-CORRECTNESS:
--   * Per-domain MRR uses the LARGEST-REMAINDER split (floor share by seats, last
--     domain absorbs the remainder) so Σ(per-domain mrr) == pool EXACTLY (no leak).
--     pool = round(ex-GST subtotal / 12) — same ex-GST basis as 0057 (bug #36).
--     This is the faithful twin of splitMrr() in src/lib/quotes/bulk.ts (unit-tested).
--   * Dues are tracked PER-ORDER (quote/invoice), so each domain sub stores
--     outstanding_amount = 0 and the subsequent-payment branch SKIPS bulk quotes
--     (never fans a single UPDATE across 1000 rows).
--   * One consolidated draft PO for the whole order (seats = Σ), not 1000 POs.
--   * Idempotent: a partial unique index on (tenant_id, quote_id, lower(domain))
--     makes re-expansion a no-op even under concurrency.
--
-- Everything for NON-bulk quotes (single new-sub path, renewal/extension path,
-- add-seats, idempotency gate, lead->customer, RV, invoice-paid, return shape) is
-- BYTE-IDENTICAL to 0057 — existing money flows are unchanged.

-- 1) Bulk-safe idempotency index (scoped to quote-linked, domained subs).
create unique index if not exists subscriptions_tenant_quote_domain_unique
  on public.subscriptions (tenant_id, quote_id, lower(domain))
  where quote_id is not null and domain is not null;

-- 2) record_payment with the bulk-expand branch.
...
begin
  ...
  -- Does the quote contain any bulk (expand-per-domain) line?
  v_has_bulk := exists (
    select 1
    ...
    if v_first_line is not null then
      ...
      if v_is_annual and v_customer_id is not null then
        ...
        if coalesce((v_first_line->>'bulk')::boolean, false)
          ...
          -- ── BULK: one subscription per domain ───────────────────────────
          ...
          -- pool = ex-GST monthly for the whole order (bug #36 basis)
          ...
          for v_d in select e from jsonb_array_elements(v_first_line->'domains') e loop
            ...
            -- largest-remainder split: last domain absorbs remainder → Σ == pool
            if v_idx < v_n then
            ...
            values (v_tenant_id, v_customer_id, v_quote.customer_name, v_plan_name, v_vendor, v_dom_seats, v_dom_mrr,
              current_date, (current_date + interval '1 year')::date, 'active', 0, v_dom, p_quote_id)
...
-- ── Verification (run on a branch / scoped, then ROLLBACK) ───────────────────
-- The per-domain MRR split mirrors splitMrr() in src/lib/quotes/bulk.ts (keep
-- both textually identical). Manual smoke test:
--   1. Seed a customer + a bulk quote: line_items = [{bulk:true, name:'Google
--      Workspace Business Starter', commitment:'annual_yearly', qty:10, rate:..,
--      domains:[{domain:'a.com',seats:5},{domain:'b.in',seats:3},{domain:'c.io',seats:2}]}],
--      subtotal = known ex-GST, amount = subtotal*1.18.
--   2. select record_payment(quote_id, amount, 'bank_transfer', 'TEST-1');
--      → assert 3 subs for the quote, Σ seats = 10, Σ mrr = round(subtotal/12),
--        all quote_id stamped, outstanding_amount = 0, one PO with seats = 10.
--   3. Re-run with same reference → idempotent_replay = true, still 3 subs.
--   4. Cross-tenant call → raises. Partial then balance on a bulk quote → no
--      outstanding fan-out, subs count stays 3.
